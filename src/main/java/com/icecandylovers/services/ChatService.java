...
public class ChatService {
    ...
    public String processarMensagem(String mensagemOriginal) {
        if (mensagemOriginal == null || mensagemOriginal.isBlank()) {
            throw new IllegalArgumentException("A mensagem não pode ser vazia");
        }
        String mensagemNormalizada = normalizar(mensagemOriginal.trim().toLowerCase());
        ...
        String respostaApi = chamarOpenAi(mensagemOriginal.trim());
        if (respostaApi.startsWith("Ops,")) {
            return fallbackResposta(mensagemNormalizada);
        }
